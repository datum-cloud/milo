...
func (p *ClaimCreationPlugin) handleAdmission(ctx context.Context, attrs admission.Attributes, o admission.ObjectInterfaces) error {
	...
	if err := p.createAndWaitForResourceClaim(ctx, policy, evalContext); err != nil {
		...
		// Use the singular form of the resource type (e.g., "project" instead of "projects")
		resourceType := attrs.GetResource().Resource
		...
		return errors.NewForbidden(gr, attrs.GetName(), fmt.Errorf("Insufficient resources available to create %s. Review your quota usage and reach out to support if you need additional resources.", resourceType))
	}
...
func (p *ClaimCreationPlugin) createResourceClaim(ctx context.Context, policy *quotav1alpha1.ClaimCreationPolicy, evalContext *EvaluationContext) (string, string, error) {
	...
	spec, err := p.templateEngine.RenderResourceClaim(ctx, policy.Spec.Target.ResourceClaimTemplate, evalContext, p.policyEngine)
	if err != nil {
		return "", "", fmt.Errorf("failed to render ResourceClaim spec: %w", err)
	}
	if len(spec.Requests) == 0 {
		p.logger.V(1).Info("No resource requests generated after template evaluation; skipping claim creation", "policy", policy.Name)
		return "", "", nil
	}

	// Render metadata (name/generateName/namespace and annotations)
	name, generateName, namespace, metaLabels, metaAnnotations, err := p.templateEngine.RenderClaimMetadata(policy.Spec.Target.ResourceClaimTemplate.Metadata, evalContext)
	if err != nil {
	...
	for key, value := range metaAnnotations {
...
func isClaimGranted(claim *unstructured.Unstructured) bool {
	return apimeta.IsStatusConditionTrue(extractConditions(claim), quotav1alpha1.ResourceClaimGranted)
}
...
func isClaimDenied(claim *unstructured.Unstructured) bool {
	cond := apimeta.FindStatusCondition(extractConditions(claim), quotav1alpha1.ResourceClaimGranted)
