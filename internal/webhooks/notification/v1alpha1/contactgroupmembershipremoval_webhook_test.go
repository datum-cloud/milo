	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			builder := fake.NewClientBuilder().WithScheme(cgrTestScheme).
				WithIndex(&notificationv1alpha1.ContactGroupMembershipRemoval{}, cgmrByContactGroupTupleKey, func(o client.Object) []string {
...
func TestContactGroupMembershipRemovalValidator_UserContext(t *testing.T) {
	makeRemoval := func(name, contact, group string) *notificationv1alpha1.ContactGroupMembershipRemoval {
		return &notificationv1alpha1.ContactGroupMembershipRemoval{
			ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"},
			Spec: notificationv1alpha1.ContactGroupMembershipRemovalSpec{
	...
	tests := map[string]struct {
		...
		"user context: user creates removal for own contact": {
			newObj:      makeRemoval("rm1", "alice-contact", "g1"),
			seedObjects: []client.Object{makeContactWithSubject("alice-contact", "alice"), makeGroup("g1")},
			userID:      "alice",
			hasUserCtx:  true,
			expectError: false,
		},
		"user context: user tries to create removal for other user's contact": {
			newObj:        makeRemoval("rm2", "bob-contact", "g1"),
...
func TestContactGroupMembershipRemovalValidator_ValidateDelete(t *testing.T) {
	makeRemoval := func(name, contact, group string) *notificationv1alpha1.ContactGroupMembershipRemoval {
		return &notificationv1alpha1.ContactGroupMembershipRemoval{
			ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "default"},
			Spec: notificationv1alpha1.ContactGroupMembershipRemovalSpec{
	...
	tests := map[string]struct {
		removal       *notificationv1alpha1.ContactGroupMembershipRemoval
		seedObjects   []client.Object
		userID        string
		hasUserCtx    bool
		expectError   bool
		...
		"user context: user deletes own removal request": {
			removal:     makeRemoval("rm1", "alice-contact", "g1"),
			seedObjects: []client.Object{makeContactWithSubject("alice-contact", "alice")},
			userID:      "alice",
			hasUserCtx:  true,
			expectError: false,
		},
		"user context: user tries to delete other user's removal request": {
			removal:       makeRemoval("rm2", "bob-contact", "g1"),
			seedObjects:   []client.Object{makeContactWithSubject("bob-contact", "bob")},
			userID:        "alice",
			hasUserCtx:    true,
			expectError:   true,
		...
		"user context: validation fails if contact is missing": {
			removal:       makeRemoval("rm4", "missing-contact", "g1"),
			seedObjects:   []client.Object{},
			userID:        "alice",
			hasUserCtx:    true,
			expectError:   true,
			errorContains: "failed to get contact for validation",
		},
	}
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			ctx := admission.NewContextWithRequest(context.Background(), req)
			_, err := v.ValidateDelete(ctx, tt.removal)
			if tt.expectError {
				assert.Error(t, err)
				if tt.errorContains != "" {
					assert.Contains(t, strings.ToLower(err.Error()), strings.ToLower(tt.errorContains))
