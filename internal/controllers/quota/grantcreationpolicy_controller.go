...
func (r *GrantCreationPolicyReconciler) validatePolicy(ctx context.Context, policy *quotav1alpha1.GrantCreationPolicy) error {
	...
	// Validate grant template structure
	if err := r.TemplateValidator.ValidateGrantTemplate(policy.Spec.Target.ResourceGrantTemplate); err != nil {
...
func (r *GrantCreationPolicyReconciler) validateResourceTypes(ctx context.Context, policy *quotav1alpha1.GrantCreationPolicy) error {
	logger := log.FromContext(ctx)

	// Get all resource registrations once
	...
	for i := range registrations.Items {
		...
		registrationMap[reg.Spec.ResourceType] = reg
	}

	// Validate each resource type in allowances
	for i, allowance := range policy.Spec.Target.ResourceGrantTemplate.Spec.Allowances {
		resourceType := allowance.ResourceType

		registration, exists := registrationMap[resourceType]
		if !exists {
			return fmt.Errorf("allowance %d: resource type '%s' is not registered - please create a ResourceRegistration for this resource type", i, resourceType)
		}
		...
		if activeCondition == nil {
			return fmt.Errorf("allowance %d: resource type '%s' is not ready - the ResourceRegistration is missing status conditions", i, resourceType)
		}

		if activeCondition.Status != metav1.ConditionTrue {
			return fmt.Errorf("allowance %d: resource type '%s' is not active - ResourceRegistration status: %s (%s)",
