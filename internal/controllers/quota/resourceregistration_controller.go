...
type ResourceRegistrationReconciler struct {
	client.Client
...
func (r *ResourceRegistrationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, err error) {
	...
	// Fetch the cluster-scoped ResourceRegistration and check for errors
	...
	// TODO: add finalizer to delete grants, claims, effective grants, allowance
	// buckets that are tied to this registration so they aren't orphaned.
	// It is unlikely registrations will be deleted, so this can be deferred to
	// a follow up PR. For now, RBAC does not allow deletion of registrations.
	...
	logger.Info("Reconciling ResourceRegistration",
		...
		"resourceTypeName", registration.Spec.ResourceTypeName,
	...
	// Get the current active condition from the status and create a new one in a
	// pending state if it doesn't already exist. Otherwise, update the observed
	// generation to match the current one being processed.
	...
	// If the registration is already active, skip the rest of the reconciliation.
	...
	if err := r.validateRegistration(ctx, &registration); err != nil {
		...
		activeCondition.Message = fmt.Sprintf("Validation failed: %v. Please check the resourceTypeName and ownerRef fields to ensure they are valid.", err)
	} else {
		...
		activeCondition.Message = "The registration is active and resource grants and claims can be created for this resource type."
	}
...
// resource type, owner ref, and dimensions are valid.
func (r *ResourceRegistrationReconciler) validateRegistration(_ context.Context, _ *quotav1alpha1.ResourceRegistration) error {
	return nil
}

func (r *ResourceRegistrationReconciler) SetupWithManager(mgr ctrl.Manager) error {
