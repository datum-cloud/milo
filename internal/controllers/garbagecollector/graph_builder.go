...
// NewDependencyGraphBuilderWithShared lets callers inject shared queues/cache/broadcaster.
// Use this when running many per-partition GraphBuilders that feed a single GC worker pool.
func NewDependencyGraphBuilderWithShared(
	ctx context.Context,
	metadataClient metadata.Interface,
	...
	// shared plumbing (can be per-GB if you want the old behavior)
	attemptToDelete workqueue.TypedRateLimitingInterface[*node],
	attemptToOrphan workqueue.TypedRateLimitingInterface[*node],
	absentOwnerCache *ReferenceCache,
	...
	if attemptToDelete == nil {
		attemptToDelete = workqueue.NewTypedRateLimitingQueueWithConfig(
			workqueue.DefaultTypedControllerRateLimiter[*node](),
			workqueue.TypedRateLimitingQueueConfig[*node]{Name: "garbage_collector_attempt_to_delete"},
		)
	}
	if attemptToOrphan == nil {
		attemptToOrphan = workqueue.NewTypedRateLimitingQueueWithConfig(
			workqueue.DefaultTypedControllerRateLimiter[*node](),
			workqueue.TypedRateLimitingQueueConfig[*node]{Name: "garbage_collector_attempt_to_orphan"},
		)
	}
	if absentOwnerCache == nil {
		absentOwnerCache = NewReferenceCache(500)
	}

	gb := &GraphBuilder{
		eventRecorder:    broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "garbage-collector-controller"}),
		...
		graphChanges: workqueue.NewTypedRateLimitingQueueWithConfig(
			...
			workqueue.TypedRateLimitingQueueConfig[*event]{Name: "garbage_collector_graph_changes"},
		),
...
// NewDependencyGraphBuilder keeps the old behavior (dedicated queues/cache).
// Under the hood it just delegates to the shared constructor.
func NewDependencyGraphBuilder(
	...
	return NewDependencyGraphBuilderWithShared(
		ctx,
		metadataClient,
		mapper,
		ignoredResources,
		sharedInformers,
		informersStarted,
		nil, // attemptToDelete (nil => allocate)
		nil, // attemptToOrphan  (nil => allocate)
		nil, // absentOwnerCache (nil => allocate)
		nil, // broadcaster      (nil => allocate)
	)
}
...
func (gb *GraphBuilder) processGraphChanges(logger klog.Logger) bool {
	...
	if found && !event.virtual && !existingNode.isObserved() {
		...
		observedIdentity := gb.identityFromEvent(event, accessor)
	...
	case (event.eventType == addEvent || event.eventType == updateEvent) && !found:
		newNode := &node{
			identity:           gb.identityFromEvent(event, accessor),
			dependents:         make(map[*node]struct{}),
	...
	case event.eventType == deleteEvent:
		...
		if event.virtual {
			...
			deletedIdentity := gb.identityFromEvent(event, accessor)
		...
		if removeExistingNode {
			...
			if len(existingNode.dependents) > 0 {
				gb.absentOwnerCache.Add(gb.identityFromEvent(event, accessor))
			}
