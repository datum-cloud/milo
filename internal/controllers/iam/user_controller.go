...
func (r *UserController) ensureOwnerReferences(ctx context.Context, user *iamv1alpha1.User) error {
	...
	if err != nil {
		...
		// PolicyBinding exists, ensure owner reference is set
		if !hasOwnerReference(policyBinding.OwnerReferences, ownerRef) {
	...
	if err != nil {
		...
		// UserPreference exists, ensure owner reference is set
		if !hasOwnerReference(userPreference.OwnerReferences, ownerRef) {
		...
		// Update PolicyBinding for UserPreference
		userPrefPolicyBindingName := fmt.Sprintf("userpreference-self-manage-%s", user.Name)
		userPrefPolicyBinding := &iamv1alpha1.PolicyBinding{}
		err = r.Client.Get(ctx, types.NamespacedName{Name: userPrefPolicyBindingName, Namespace: "milo-system"}, userPrefPolicyBinding)
		if err != nil {
			if apierrors.IsNotFound(err) {
				// PolicyBinding doesn't exist, webhook should have created it
				log.Info("UserPreference PolicyBinding not found, skipping (webhook should create it)", "user", user.Name, "policyBinding", userPrefPolicyBindingName)
			} else {
				return fmt.Errorf("failed to get user preference policy binding: %w", err)
			}
		} else {
			// PolicyBinding exists, ensure owner reference is set
			if !hasOwnerReference(userPrefPolicyBinding.OwnerReferences, ownerRef) {
				userPrefPolicyBinding.OwnerReferences = append(userPrefPolicyBinding.OwnerReferences, ownerRef)
				if err := r.Client.Update(ctx, userPrefPolicyBinding); err != nil {
