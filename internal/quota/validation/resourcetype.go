...
func NewResourceTypeValidator(dynamicClient dynamic.Interface) ResourceTypeValidator {
	return NewResourceTypeValidatorAsync(dynamicClient)
}

// NewResourceTypeValidatorAsync creates a ResourceTypeValidator that starts asynchronously.
// This is useful for admission plugins where initialization time is very limited.
// The validator will return immediately and sync in the background.
// Validation requests before sync completion will fall back to direct API calls.
func NewResourceTypeValidatorAsync(dynamicClient dynamic.Interface) ResourceTypeValidator {
	logger := log.Log.WithName("resource-type-validator")
	logger.Info("Creating async ResourceTypeValidator with delayed start")
	...
	logger.Info("Async ResourceTypeValidator created, will initialize when API server is ready")
	return validator
}
