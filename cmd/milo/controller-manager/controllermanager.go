...
func Run(ctx context.Context, c *config.CompletedConfig, opts *Options) error {
	...
	run := func(ctx context.Context, controllerDescriptors map[string]*ControllerDescriptor) {
		...
		if opts.ControlPlane.Scope == controlplane.ScopeCore {
			...
			ctrlConfig.QPS = 100    // Increase from default ~20 to handle quota operations
			ctrlConfig.Burst = 200  // Increase from default ~30 to handle quota operations
			...
			quotaDynamicConfig.QPS = 500   // Very high rate limits for quota operations to prevent throttling
			quotaDynamicConfig.Burst = 1000 // Very high burst for quota operations to prevent throttling
