...
func (gc *GarbageCollector) getMetadataForIdentity(id objectReference) (metav1.Object, error) {
	...
	apiResource, _, err := apiResourceUsing(mapper, id.APIVersion, id.Kind)
	if err != nil {
		return nil, err
	}
	...
	if !ok {
		return nil, fmt.Errorf("expect a runtime.Object, got %T", raw)
	}
...
func (gc *GarbageCollector) deleteOwnerRefJSONMergePatch(item *node, ownerUIDs ...types.UID) ([]byte, error) {
	accessor, err := gc.getMetadataForIdentity(item.identity)
	if err != nil {
		return nil, err
	}
	expectedObjectMeta := ObjectMetaForPatch{ResourceVersion: accessor.GetResourceVersion()}
...
func (gc *GarbageCollector) unblockOwnerReferencesJSONMergePatch(n *node) ([]byte, error) {
	accessor, err := gc.getMetadataForIdentity(n.identity)
	if err != nil {
		return nil, err
	}
	expectedObjectMeta := ObjectMetaForPatch{ResourceVersion: accessor.GetResourceVersion()}
	...
	for _, owner := range n.owners {
		...
		expectedObjectMeta.OwnerReferences = append(expectedObjectMeta.OwnerReferences, owner)
	}
	return json.Marshal(objectForPatch{expectedObjectMeta})
}
