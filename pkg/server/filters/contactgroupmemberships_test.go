...
func TestUserContactGroupMembershipListConstraintDecorator(t *testing.T) {
	testCases := []struct {
		name                  string
		requestPath           string
		apiGroup              string
		...
		{
			name:                  "contactgroupmemberships list with user context",
		...
		},
		{
			...
			requestPath:           "/apis/notification.miloapis.com/v1alpha1/contactgroupmemberships",
			apiGroup:              notificationv1alpha1.SchemeGroupVersion.Group,
			resource:              "contactgroupmemberships",
			verb:                  "list",
			userID:                "test-user",
		...
		{
			name:        "contactgroupmemberships get request",
			requestPath: "/apis/notification.miloapis.com/v1alpha1/contactgroupmemberships/test-membership",
			apiGroup:    notificationv1alpha1.SchemeGroupVersion.Group,
			resource:    "contactgroupmemberships",
			verb:        "get",
			userID:      "test-user",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			handler := UserContactGroupMembershipListConstraintDecorator(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
